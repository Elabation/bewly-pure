...
def section_statdig(d):
    ...
    <p><b>倒 U 回顾性确认：峰值在种子 CBI 9-12（39.7%），超级神作区（12+）塌回 24.2%</b>。Elabation 的
    「掐头去尾」直觉的精确版本：该掐的是<b>种子的头</b>（病毒式超级神作吸引泛化路人），中坚神作
    （6-12）的评论区才是行家聚集地。三处独立证据收敛：探针 7 人强度（6.47-9.91）全部落在峰区、
    工程链头部的塌陷、分带尖峰。<b>但前瞻实验未复现</b>（预注册：中带播种 26.2%，触发失败线≤0.28）——
    峰区的 6 个贡献用户属聚簇噪声。回顾假说留档，前瞻否决：这一对就是统计学的呼吸。</p>
    <div class="geo">
...
不等于「更有品」——超级神作（CBI 18.3）的评论区聚集的是泛化路人。Elabation 据此提出假说
<b>「掐头去尾」</b>（强度两端都剪、只挖中带），并已完成<b>预注册前瞻实验</b>：18 粒种子全部随机抽自
CBI∈[6,12) 中带（协议执行无偏差），46 用户 / 680 合格视频，神作率 <b>26.2%</b>——落在随机补足基线
的自助 CI [0.194, 0.301] 之内，触发预注册失败线（≤0.28）。<b>裁决：回顾性响应曲线的峰区属聚簇噪声，
掐头去尾作为播种策略不采纳</b>（预注册文件 midband_prereg_verdict.json 保留；若未来以 3× 样本重测，
须重新注册）。</p>
...
26.2%，触发失败线≤0.28）——回顾峰区属聚簇噪声，掐头去尾不采纳，预注册文件留档；
